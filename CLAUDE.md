...
4. `password.yml` (only when `paseo_set_password`, default true) — see below.
5. `config.yml` — deep-merges only owned keys into `config.json`: `daemon.listen`, `daemon.hostnames`, `daemon.relay.enabled`, `worktrees.root`. Everything else (the password hash, unknown keys) survives.
6. `opencode.yml` — deploy the sanitized config (+ optional `auth.json` from vault), and the optional discover/fetch/MCP-merge/print paths.
7. `fetch_daemon_config.yml` (only when `paseo_fetch_daemon_config`, default false), `selinux.yml` (`paseo_manage_selinux` + RedHat + SELinux enabled: relabel binaries `bin_t`), then `service.yml` — env file (only when `paseo_opencode_secrets` is non-empty; units tolerate its absence), systemd unit or OpenRC init.d + conf.d, flush handlers, enable+start.
